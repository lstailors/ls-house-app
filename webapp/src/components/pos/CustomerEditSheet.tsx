  ...
  return (
    <div className="card-glass p-4 flex items-center gap-3">
      <button
        ...
        onClick={onProfile}
        disabled={!onProfile}
        className="w-12 h-12 rounded-full bg-forest-raised border border-brass/30 grid place-items-center display text-lg text-brass-light shrink-0 overflow-hidden disabled:cursor-default"
        title={onProfile ? "Open profile" : undefined}
      >
        {imageUrl ? (
      ...
      <div className="min-w-0 flex-1">
        {onProfile ? (
          <button type="button" onClick={onProfile} className="font-semibold text-base truncate text-left hover:text-brass-light transition-colors w-full">
            {name}
          </button>
        ) : (
      ...
      <div className="flex flex-col sm:flex-row gap-2 shrink-0">
        {onProfile && (
          <button type="button" onClick={onProfile} className="btn-ghost h-11 px-4 text-[12px]">
            Profile
          </button>
        )}
        {onEdit && (
