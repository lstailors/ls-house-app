...
export function PriceCard({
  ...
  const { fabric, fabricCost, laborLabel, laborCost, upcharges, upchargeTotal, subtotal } = breakdown;
    ...
    >
      ...
      <div className="flex items-end justify-between mb-3">
        <span className="ui-label text-xs">
          {priceTbd ? "Reference price" : "Total"}
        </span>
        <span
          className={cn(
      ...
      {upchargeTotal > 0 ? (
        <div className="text-[10px] text-cream-dim text-right -mt-2 mb-2">
          incl. {formatUSD(upchargeTotal)} style upcharges
        </div>
      ) : null}
      ...
      {priceTbd ? (
        <div className="rounded-lg border border-signal-amber/30 bg-signal-amber/5 p-3 mb-4">
          <div className="flex items-start gap-2">
            ...
            <div>
              <div className="text-xs text-cream font-medium">Quote will be saved for review</div>
              <div className="text-[11px] text-cream-muted mt-0.5">
                No deposit charged. Master tailor will price within 48 hours.
              </div>
            </div>
          </div>
        </div>
      ...
      {!priceTbd ? (
        <>
          <div className="mb-3">
            <Label className="ui-label text-[10px] mb-1.5 block">Deposit</Label>
            <div className="grid grid-cols-4 gap-1.5 mb-2">
              {[0.25, 0.5, 0.75, 1].map((pct) => {
                const amt = Math.round((subtotal * pct) / 50) * 50;
      ...
      {priceTbd ? (
        <Button
          className="w-full bg-signal-amber hover:bg-signal-amber/90 text-forest font-medium"
        ...
        >
          ...
          {depositAmount === 0
            ? "Set a deposit to charge"
            : `Charge ${formatUSD(depositAmount)} deposit`}
        </Button>
      )}

      {!priceTbd && depositAmount === 0 && subtotal > 0 ? (
        <button
          type="button"
          onClick={onSaveQuote}
          disabled={!canSubmit || isSubmitting}
          className="w-full mt-2 text-[11px] text-cream-dim hover:text-cream-muted transition-colors"
        >
          or save without deposit →
        </button>
      ) : null}

      <div className="mt-4 pt-3 border-t border-brass/10 flex items-center gap-1.5 text-[10px] text-cream-dim">
        <Sparkles className="h-3 w-3" />
        <span>Pricing builds from Fabric Pricing &amp; Style Library</span>
      </div>
    </GlassCard>
  );
}
