      ...
      {MTM_STATUSES.map((s) => {
        ...
        return (
          <button
            ...
            className={cn(
              "rounded-full border font-semibold tracking-[0.06em] uppercase whitespace-nowrap transition-colors",
              compact
                ? "h-11 min-h-[44px] px-2.5 text-[9px]"
                : "h-11 min-h-[44px] px-3 text-[10px]",
              on
                ? "bg-brass/20 border-brass text-brass-light"
                : "border-brass/20 bg-forest-raised/40 text-cream-dim hover:border-brass/45 hover:text-cream",
              !editable && "opacity-80",
            )}
          >
            {busy ? "…" : s.key}
