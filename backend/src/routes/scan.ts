...
scanRouter.get("/:token", async (c) => {
  ...
  if (!erpDoc) return c.json({ error: { message: "Not found" } }, 404);

  const isDelivered = erpDoc.lsh_status === "Delivered";

  const photos = (erpDoc.lsh_photos ?? [])
  ...
  const proofUrls = {
    ...
    signature: erpDoc.lsh_signature_image_url
      ? erpFileAbsoluteUrl(erpDoc.lsh_signature_image_url)
      : null,
  };
