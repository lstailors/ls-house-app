...
const updateCallLog = mock(async (_id: string, _doc: unknown) => ({}));
const insertSmsMessage = mock(async (_doc: unknown) => ({}));

mock.module("./comms", () => ({ matchCustomerByPhone, logErpCommunication }));
mock.module("../lib/erpnext/agents", () => ({ insertCallLog, updateCallLog, insertSmsMessage }));
...
describe("POST /unifi n8n forwarding", () => {
  ...
  test("stores emergency calls with emergency status", async () => {
    const response = await postUnifi("emergency", { call_id: "emergency-123", from: "+12125550199" });

    expect(response.status).toBe(200);
    expect(updateCallLog).toHaveBeenCalledWith(
      "emergency-123",
  ...
  test("uses one customer match and one forward path for SMS", async () => {
    ...
    expect(matchCustomerByPhone).toHaveBeenCalledWith("+12125550123");
  ...
  test("silently skips forwarding when the n8n URL is unset", async () => {
    ...
    const response = await postUnifi("missed", { call_id: "missed-123", from: "+12125550123" });
  ...
  test("rejects a bogus UniFi token before storage or forwarding", async () => {
    ...
    const response = await postUnifi(
      "transcript",
      { call_id: "blocked-123", from: "+12125550123" },
      "bogus-token",
    );
  ...
  test("does not wait for the n8n request to settle", async () => {
    const pending = new Promise<Response>(() => {});
    const forward = mock(() => pending);
    globalThis.fetch = forward as unknown as typeof fetch;

    const outcome = await Promise.race([
      Promise.resolve(postUnifi("voicemail", { call_id: "voicemail-123", from: "+12125550123" })).then(
        () => "response",
      ),
  ...
  test("keeps the UniFi response successful when n8n rejects", async () => {
    ...
    try {
      const response = await postUnifi("failed", { call_id: "failed-123", from: "+12125550123" });
      await Bun.sleep(0);
