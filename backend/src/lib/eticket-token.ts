 ...
 *
 * Edge-safe: pure JS SHA-256 (no node:crypto) so Vercel Edge can bundle
 * the intake-alterations graph.
 */
...
/** Minimal SHA-256 → hex (sync, Edge-safe). */
function sha256Hex(message: string): string {
  ...
  // length in bits as 64-bit big-endian (we only need low 32 for our sizes)
...
export function eTicketKey(ticketName: string): string {
  ...
  return sha256Hex(raw).slice(0, 20);
}
...
export function eTicketPublicUrl(ticketName: string): string {
  const base = (
    process.env.ALTS_URL ||
    process.env.VITE_ALTS_PUBLIC_URL ||
