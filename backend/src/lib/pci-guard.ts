...
export function findPanHits(raw: string | null | undefined): PanHit[] {
  ...
  const t1Pan = t1?.[1];
  if (t1 && t1Pan && luhnOk(t1Pan)) {
    hits.push({ kind: "track1", last4: last4(t1Pan), index: t1.index });
  }
  const t2 = TRACK2_RE.exec(text);
  const t2Pan = t2?.[1];
  if (t2 && t2Pan && luhnOk(t2Pan)) {
    hits.push({ kind: "track2", last4: last4(t2Pan), index: t2.index });
  }
